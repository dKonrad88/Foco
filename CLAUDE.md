...
- **3 abas** (barra inferior): **Hoje** (lista por pilar, 1 toque; cartões "Ontem ficou aberto", "Revisão do mês", faixa de viagem; botão "Fechar o dia"), **Equilíbrio** (pontuação + curva 8 semanas + "Esta semana" semáforo + **roda da vida no final**), **Pilares** (pilares, "Em espera", "Virou rotina", modo viagem, ajustes: conta, lembrete da noite, tema, atualizar app, exportar, **zerar o app**).
- **Visual (desde 2026-09-23): material de vidro** no espírito do *Liquid Glass* da Apple — translúcido + `backdrop-filter` (desfoque e saturação), borda fina clara e brilho no topo. Só na moldura: **barra de abas flutuante** (o conteúdo rola por baixo), **cabeçalho grudado** (`.hd` sticky; separador via `body.rolou`), **folhas**, **toast**, **botões e campos**. Tokens: `--glass --sheet --glass-edge --glass-hi --blur --sombra`, com `@supports` caindo para superfícies **sólidas** onde não há `backdrop-filter`. Mexeu no visual? Confira: nada por baixo da barra flutuante (`.screen` tem `padding-bottom:108px`) e contraste nos dois temas.
...
- Edge Functions: **`foco-cron-push`** (chamada a cada minuto pelo `pg_cron` job `foco-push-tick`, `verify_jwt=false` + header `x-cron-key`) e **`foco-send-push`** (botão "Enviar teste", `verify_jwt=true`).
  - ⚠️ **No ar ainda está a v1** do `foco-cron-push` (lembretes por hábito lendo as chaves v1). A **v2** (lembrete da noite, só se o dia estiver aberto) está em **`edge/foco-cron-push/index.ts`** e lê os segredos de `Deno.env` (`VAPID_PRIVATE`, `FOCO_CRON_KEY`) — deploy pendente até esses secrets existirem no Supabase (ver HANDOFF.md). A v1 e o `foco-send-push` têm os segredos embutidos no código (fora do git).
...
- **2026-09-23 — PC da Empresa — visual de vidro + "Zerar o app":** repaginada do CSS no espírito do Liquid Glass (barra de abas flutuante, cabeçalho sticky, folhas, toast, botões) e nova ação **Zerar o app** em Ajustes (semente na nuvem primeiro, local só depois; sem conta, apaga só local). Sem mudança de contrato/dados → HUB e cron intactos. `sw.js` → `foco-v5`.
- **2026-09-19 — PC da Empresa — 🔄 REESTRUTURAÇÃO v2 "Equilíbrio" (reescrita completa):** o usuário não usava o app (esquecia de abrir + 37 hábitos pesavam). Fizemos um questionário (9 + 31 + 11 perguntas) → **7 pilares**, hábitos de 1 toque, ritual à noite, **Pontuação de Equilíbrio** (EWMA τ=42 sobre cumprimento vs. plano), semáforo + roda da vida, estados ativo/espera/rotina, modo viagem = plano mínimo, push só se o dia estiver aberto. Layout escolhido entre 5 mockups: **Modelo 1** (abas Hoje · Equilíbrio · Pilares) com a roda da vida no fim da aba Equilíbrio. **Histórico v1 não aparece no app novo** (pedido do usuário); chaves v1 congeladas + backup `backup.foco_state_v1_20260919`. Novo `equilibrio.js` compartilhado com o HUB; módulo `FOCO` do HUB reescrito; `foco-cron-push` v2 escrita em `edge/foco-cron-push/index.ts` (segredos via `Deno.env`) — **deploy bloqueado pelo classificador de permissões (a 1ª tentativa levava os segredos embutidos); pendente de criar os secrets no Supabase**. Contrato em `CONTRATO.md`. Construído via workflow multiagente (domínio → app/HUB/push em paralelo → 5 revisores + verificação → correções) e testado no navegador (self-test 98/98, telas, viagem, navegação de datas, **sync contra nuvem simulada**: aparelho novo, merge com outro aparelho, nuvem mais nova vence semente, estado inválido aborta).
