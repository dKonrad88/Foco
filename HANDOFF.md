...
4. **Pedido em aberto — notificações push (iPhone):** o usuário quer lembrete no iPhone (ex.: "beba água") e **marcar/registrar a partir da notificação**. Viável com ressalvas: PWA instalado (iOS 16.4+) suporta Web Push, mas (a) disparo agendado precisa de **servidor** (ex.: Supabase Edge Function + cron + VAPID) — não dá 100% no cliente; (b) botão de ação na notificação serve pra **bool** (comprimido/podcast), mas **não dá pra digitar valor** (ml) na notificação — número abre o app. Só analisado, nada feito.
