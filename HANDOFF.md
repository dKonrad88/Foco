...
4. **Push (iPhone) — PILOTO construído, aguardando o usuário testar no device:** menor cadeia de Web Push, só pra provar que push chega no iPhone dele.
   - **Infra criada (Supabase `jlouesrrmqeauzlgvrpw`, mesmo do HUB — aditivo, prefixo `foco_`):** tabela **`foco_push_subs`** (`user_id,endpoint,p256dh,auth,ua`; RLS `own_push_subs`) + Edge Function **`foco-send-push`** (`verify_jwt=true`, CORS ok, usa `npm:web-push`, lê as subs do user pelo JWT, apaga sub 404/410). **VAPID:** pública no `index.html` (`VAPID_PUBLIC`); **privada embutida SÓ na Edge Function** (nunca no git — confirmado). Idealmente mover a privada pra secret depois.
   - **No app:** Ajustes → **"Ativar notificações"** (`pushEnable`: exige standalone + login; pede permissão; `pushManager.subscribe`; salva a sub no Supabase) e **"Enviar notificação de teste"** (`pushTest`: `sb.functions.invoke('foco-send-push')`). `sw.js` (cache→**`foco-v2`**) ganhou handlers `push` (mostra a notificação) e `notificationclick` (foca/abre o Foco).
   - **Testado por mim:** sintaxe, CORS/preflight (200) e auth (401 sem JWT) da função, botões e os 3 guards do cliente. **Falta só o teste real no iPhone** (subscribe + push chegando) — só o usuário faz.
   - **Ainda falta pro produto final:** agendar por horário (cron/pg_cron chamando a função) e **marcar da notificação** (botão de ação p/ bool; número abre o app — digitar ml na notificação não dá). Ver análise no corpo abaixo.
